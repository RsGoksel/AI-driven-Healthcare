...
def build_one(md_path: Path, pdf_path: Path, lang: str,
              also_docx: bool = False) -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        if res.returncode != 0:
            ...
            raise SystemExit(1)
        print(f"    wrote {pdf_path.stat().st_size / 1024:.0f} KB")

        # docx output for editable academic delivery (no LaTeX preamble)
        if also_docx:
            ...
            print(f"  [build] {md_path.name} -> {docx_path.name}  ({lang})")
            res = subprocess.run(cmd_docx, capture_output=True, text=True)
            if res.returncode != 0:
                print(f"  [warn] docx pandoc failed:\n{res.stderr}",
                      file=sys.stderr)
            else:
                print(f"    wrote {docx_path.stat().st_size / 1024:.0f} KB")
...
def main() -> None:
    ...
    # English versions also get .docx for editable advisor delivery.
    build_one(REPORTS / "abstract_a_en.md",
              REPORTS / "abstract_a_en.pdf", lang="en", also_docx=True)
    build_one(REPORTS / "abstract_a_tr.md",
              REPORTS / "abstract_a_tr.pdf", lang="tr", also_docx=True)
    build_one(REPORTS / "abstract_b_en.md",
              REPORTS / "abstract_b_en.pdf", lang="en", also_docx=True)
    build_one(REPORTS / "abstract_b_tr.md",
              REPORTS / "abstract_b_tr.pdf", lang="tr", also_docx=True)
